...
4. Decide how the provided corpus will be represented as a local directory or manifest. Do not add web scrapers unless the human explicitly wants a scraper for their corpus.
...
7. Generate normalized description JSONL for the corpus. Mood, tags, actions, and use cases should come from this main LLM-generated description JSON.
...
9. Run the local proxy with `python3 scripts/local_proxy.py` and the web UI with `cd web && pnpm install && pnpm dev`. The proxy is for the browser UI so `ANTFLYDB_API_KEY` stays local.
